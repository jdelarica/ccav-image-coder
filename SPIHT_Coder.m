function out = SPIHT_Coder(I_Wavedata, max_bits, ~, level)
% 
% SPIHT Coder Algorithm
% 
% Encoder
%
% input:    I_Wavedata : input image in wavelet domain
...
n_max = floor(log2(abs(max(max(I_Wavedata)'))));
Bits_Header = 0;
Bits_LSP = 0;
Bits_LIP = 0;
Bits_LIS = 0;
...
out(1,[1 2 3]) = [size(I_Wavedata,1) n_max level]; bitctr = bitctr + 24;
index = 4;
Bits_Header = Bits_Header + 24;

%-----------   Initialize LIP, LSP, LIS   ----------------
temp = [];
bandsize = 2.^(log2(size(I_Wavedata, 1)) - level + 1);
temp1 = 1 : bandsize;
for i = 1 : bandsize
    temp = [temp; temp1];
end
...
fprintf('----------------                    STEP 2: Sorting Pass                ----------------\n');
    ...
    for i = 1:size(LIPtemp,1)
        ...
        if abs(I_Wavedata(LIPtemp(i,1),LIPtemp(i,2))) >= 2^n % 1: positive; 0: negative
            out(index) = 1; bitctr = bitctr + 1;
            index = index +1; Bits_LIP = Bits_LIP + 1;
            sgn = I_Wavedata(LIPtemp(i,1),LIPtemp(i,2))>=0;
    ...
    while ( i <= size(LIStemp,1))
        ...
        if LIStemp(i,3) == 0
            ...
            max_d = SPIHT_Descendant(LIStemp(i,1),LIStemp(i,2),LIStemp(i,3),I_Wavedata);
            if max_d >= 2^n
                out(index) = 1; bitctr = bitctr + 1;
                ...
                if abs(I_Wavedata(2*x-1,2*y-1)) >= 2^n
                    LSP = [LSP; 2*x-1 2*y-1];
                    out(index) = 1; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                    sgn = I_Wavedata(2*x-1,2*y-1)>=0;
                    out(index) = sgn; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                else
                    out(index) = 0; bitctr = bitctr + 1;
                ...
                if abs(I_Wavedata(2*x-1,2*y)) >= 2^n
                    LSP = [LSP; 2*x-1 2*y];
                    out(index) = 1; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                    sgn = I_Wavedata(2*x-1,2*y)>=0;
                    out(index) = sgn; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                else
                    out(index) = 0; bitctr = bitctr + 1;
                ...
                if abs(I_Wavedata(2*x,2*y-1)) >= 2^n
                    LSP = [LSP; 2*x 2*y-1];
                    out(index) = 1; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                    sgn = I_Wavedata(2*x,2*y-1)>=0;
                    out(index) = sgn; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                else
                    out(index) = 0; bitctr = bitctr + 1;
                ...
                if abs(I_Wavedata(2*x,2*y)) >= 2^n
                    LSP = [LSP; 2*x 2*y];
                    out(index) = 1; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                    sgn = I_Wavedata(2*x,2*y)>=0;
                    out(index) = sgn; bitctr = bitctr + 1;
                    index = index +1; Bits_LIS = Bits_LIS + 1;
                else
                    out(index) = 0; bitctr = bitctr + 1;
                ...
                if ((2*(2*x)-1) < size(I_Wavedata) & (2*(2*y)-1) < size(I_Wavedata))
...
% fprintf('----------------                    STEP 2: Sorting Pass                ----------------\n');
fprintf('----------------                  STEP 3: Refinement Pass               ----------------\n');
    temp = 1;
    value = floor(abs(2^(n_max-n+1)*I_Wavedata(LSP(temp,1),LSP(temp,2))));
